...
export default function Resources() {
  ...
  function goToNextPage() {
    if (
      currentPage ==
      Math.ceil(filteredResources.length / NUMBER_OF_RESOURCES_PER_PAGE)
    )
      return;
    setCurrentPage((currentPage) =>
      currentPage <
  ...
  function getResources() {
    ...
    if (filteredResources.length === 0)
      return <div>No match found for the searched term</div>;

    const resourcesInCurrentPage = filteredResources.slice(
      (currentPage - 1) * NUMBER_OF_RESOURCES_PER_PAGE,
      currentPage * NUMBER_OF_RESOURCES_PER_PAGE
    );
    ...
    return resourcesInCurrentPage.map((resource) => (
      <div className="resource" key={resource.id}>
        ...
        <div className="resource-content">
          <h2>{resource.name}</h2>
          <p>
            {truncateString(resource.description, MAX_DESCRIPTION_LENGTH, () =>
